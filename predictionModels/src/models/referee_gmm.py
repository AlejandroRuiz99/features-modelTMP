"""
referee_gmm.py — Inferencia del modo arbitral en predictionModels.
...
NO ES RESPONSABILIDAD DE ESTA PIEZA:
  - Ajustar (fit) el GMM desde historico. Eso lo hace featuresGenerator/model/core/referee_gmm.py.
  - Los parametros mu/sigma/peso del GMM llegan ya calculados en el feature dict
    bajo arbitro.estadisticas.

Flujo en predictionModels:
...
class RefereeProfile:
    ...
    (calculados por featuresGenerator).
    """
    name: str
    n_matches: int = 0
...
class ModeSelector(nn.Module):
    ...
    def __init__(self, hidden_dims: list[int] = None, dropout: float = 0.1):
        super().__init__()
...
class RefereeProfiler:
    """
    Gestor de perfiles de arbitros para predictionModels.

    A diferencia de featuresGenerator, este profiler NO ajusta GMMs desde historico.
