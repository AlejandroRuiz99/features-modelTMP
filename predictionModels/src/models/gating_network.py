...
class DynamicEnsembleWeighter:
    ...
    def __init__(
        ...
        self.gating = GatingNetwork(hidden_dims, dropout, temperature, min_weight=min_weight)
        self.lr = lr
        self.epochs = epochs
    ...
    def build_gate_features(
        ...
        foul_market_divergence = np.clip(abs(foul_market_mean - model_mean) / 10.0, 0.0, 1.0) * has_odds

        xfouls_total = float(match.get("xfouls_home", 12.5)) + float(match.get("xfouls_away", 12.5))
        raw_total = (
            float(match.get("home_fouls_committed_curr", 12.0))
            + float(match.get("away_fouls_committed_curr", 12.0))
        )
    ...
    def fit(
        ...
        anfis_firings: Optional[list[float]] = None,
    ):
        """
