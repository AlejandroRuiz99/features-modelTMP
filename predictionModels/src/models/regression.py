...
class FoulRegressionPredictor:
    ...
    def fit(self, matches: list[dict]):
        ...
        offsets = np.array([
            np.log(max(m.get("referee_expected_fouls", ref_base), 1.0)) - np.log(ref_base)
            for m in matches
        ])
    ...
    def predict_params(self, match: dict) -> tuple[float, float]:
        ...
        offset = torch.tensor([np.log(max(ref_expected, 1.0)) - np.log(ref_base)],
                              dtype=torch.float32)
...
class TeamFoulRegressor:
    ...
    def fit(self, matches: list[dict]):
        home_matches = [
            {**m, "fouls_total": m["fouls_home"]} for m in matches
        ]
        away_matches = [
            {**m, "fouls_total": m["fouls_away"]} for m in matches
        ]
