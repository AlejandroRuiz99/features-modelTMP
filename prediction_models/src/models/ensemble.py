...
class FoulPredictionEnsemble:
    ...
    def fit(
        ...
        gating_matches: list[dict] | None = None,
    ):
        """
        ...
        Staged training:
            1. Registrar perfiles GMM de arbitros desde los feature dicts
                (los parametros GMM vienen pre-calculados en features_generator)
        ...
        # Los parametros GMM llegan ya calculados desde features_generator.
        ...
        self.layer_bayes.fit(
            enriched, team_avg_committed, team_avg_suffered, team_avg_rank
        )
        ...
        self.weighter.fit(gating_enriched, pmfs_b, pmfs_r, pmfs_a, actual, firings)
        ...
        if self.config.get("variance_posthoc_scale") is None:
            self._variance_posthoc_scale = self._estimate_variance_scale(
                gating_enriched
            )
    ...
    def _estimate_variance_scale(self, matches: list[dict]) -> float:
        ...
        for m in matches:
            try:
                ...
                logger.warning(
                    "_estimate_variance_scale: failed to compute variance for a match",
                    exc_info=True,
                )
                continue
        if len(preds) < 10:
    ...
    def _enrich_match(self, match: dict) -> dict:
        ...
        strict_prob = self.referee_profiler.predict_mode(
            ...
            season_phase,
            home_is_top,
            aggressiveness_norm_total=float(m.get("aggressiveness_norm_total", 0.5)),
            urgency_home=float(m.get("urgency_home", 0.5)),
            urgency_away=float(m.get("urgency_away", 0.5)),
            ref_pair_delta_sum=float(m.get("ref_pair_delta_sum", 0.0)),
            pace_index_curr=float(m.get("pace_index_curr", 31.0)),
    ...
    def predict(self, match: dict) -> MatchPrediction:
        ...
        if abs(self._variance_posthoc_scale - 1.0) > 1e-6:
            pmf_combined = scale_pmf_variance(
    ...
    def _heuristic_team_split(
        ...
        home_fouls = float(
            match.get("xfouls_home") or match.get("home_fouls_committed_avg", 12.5)
        )
        away_fouls = float(
            match.get("xfouls_away") or match.get("away_fouls_committed_avg", 12.5)
        )
    ...
    def predict_team_fouls(
        self,
        match: dict,
        total_prediction: MatchPrediction | None = None,
        reconcile: bool = True,
    ) -> dict | None:
        ...
        return {
            ...
            "raw_home_expected": raw_home_mu,
            "raw_away_expected": raw_away_mu,
            "coherent_constraint_error": float(
                total_pmf.mean - home_pmf.mean - away_pmf.mean
            ),
            "reconciled": True,
        }
    ...
    def save(self, directory: str | Path):
        ...
        torch.save(
            self.layer_regression.model.state_dict(), directory / "regression.pt"
        )
