...
# ---------------------------------------------------------------------------
# Constantes del modelo ICC y factores de ajuste
# ---------------------------------------------------------------------------

# Pesos del ICC (Indice de Competitividad Contextual)
ICC_WEIGHT_URGENCY = 0.40  # Peso de urgency_score
ICC_WEIGHT_MOMENTUM = 0.25  # Peso de momentum
ICC_WEIGHT_FATIGUE = 0.20  # Peso de fatigue_score (negativo)
ICC_WEIGHT_ROTATION = 0.15  # Peso de rotation_score (negativo)

# Pesos para urgency compuesta
URGENCY_WEIGHT_BASE = 0.60  # Peso del urgency_base (objetivo declarado)
URGENCY_WEIGHT_PRESSURE = 0.40  # Peso de pressure_score (cercania al objetivo)

# Factores de ajuste por ICC para features del modelo
ICC_XG_FACTOR = 0.10  # xG_factor = 1.0 + ICC_XG_FACTOR * icc
ICC_XFOULS_FACTOR = 0.12  # xFouls_factor = 1.0 + ICC_XFOULS_FACTOR * icc
ICC_POSESION_SCALE = 2.0  # posesion_delta_pp = ICC_POSESION_SCALE * icc
...
def _build_league_table(season_games: list[dict]) -> dict[str, dict]:
    ...
    for p in season_games:
        ...
        tabla[h]["gf"] += hg
        tabla[h]["ga"] += ag
        tabla[h]["gd"] = tabla[h]["gf"] - tabla[h]["ga"]
        tabla[h]["played"] += 1
        tabla[a]["gf"] += ag
        tabla[a]["ga"] += hg
        tabla[a]["gd"] = tabla[a]["gf"] - tabla[a]["ga"]
        tabla[a]["played"] += 1
        if hg > ag:
            tabla[h]["points"] += 3
        elif hg < ag:
...
def _calc_icc(
    ...
    if urgency_score is None:
        return None
    icc = (
        ICC_WEIGHT_URGENCY * urgency_score
        + ICC_WEIGHT_MOMENTUM * momentum
        - ICC_WEIGHT_FATIGUE * fatigue_score
        - ICC_WEIGHT_ROTATION * rotation_score
    )
    return round(max(-1.0, min(1.0, icc)), 3)


def _icc_label(icc: float | None) -> str | None:
    if icc is None:
        return None
    return "alto" if icc >= 0.2 else "bajo" if icc <= -0.2 else "medio"


def _urgencia_label(urgency_score: float | None) -> str | None:
    if urgency_score is None:
        return None
    return (
        "alta" if urgency_score >= 0.7 else "media" if urgency_score >= 0.45 else "baja"
    )
...
def _same_referee(a: str | None, b: str | None) -> bool:
    na, nb = norm_text(a), norm_text(b)
    return bool(na and nb and (na == nb or na in nb or nb in na))


def _fouls_in_match(match: dict, team_name: str) -> float | None:
...
def _build_team_context(
    *,
    ...
    objetivo_label, urgency_base, competiciones_activas = _get_objective(
        team_name, objectives
    )

    if urgency_base is not None:
        pressure_score, pressure_meta = _calc_objective_pressure(
            objetivo_label,  # type: ignore[arg-type]
            table,
            team_name,
        )
        urgency_score = round(
            URGENCY_WEIGHT_BASE * urgency_base
            + URGENCY_WEIGHT_PRESSURE * pressure_score,
            3,
        )
        rotation_score = _calc_rotation_score(
            cal["days_to_next"],
            cal["next_competition"],
        ...
        rotation_score = _calc_rotation_score(
            cal["days_to_next"], cal["next_competition"], False
        )

    icc = _calc_icc(
        urgency_score, forma["momentum_score"], fatigue_score, rotation_score
    )

    # Factores de ajuste: neutros (1.0 / 0.0) si no hay ICC
    factors = {
        "xg_factor": round(1.0 + ICC_XG_FACTOR * icc, 4) if icc is not None else 1.0,
        "xfouls_factor": round(1.0 + ICC_XFOULS_FACTOR * icc, 4)
        if icc is not None
        else 1.0,
        "posesion_delta_pp": round(ICC_POSESION_SCALE * icc, 3)
        if icc is not None
        else 0.0,
    }
