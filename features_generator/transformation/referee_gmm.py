...
PREDICCION (inferencia de modo en prediction_models).
...
def _fit_with_shrinkage(
    ...
    try:
        ...
        logger.debug(
            "GMM fallback in _fit_with_shrinkage for %s", nombre, exc_info=True
        )
...
def calcular_perfiles_gmm(
    ...
    try:
        ...
        logger.debug("GMM fallback: global fit failed, using defaults", exc_info=True)
    ...
    for nombre, dated in series.items():
        ...
        if len(dated) >= MIN_MATCHES_STANDALONE:
            try:
                ...
                logger.debug(
                    "GMM fallback: fit failed for referee %s, applying shrinkage",
                    nombre,
                    exc_info=True,
                )
...
def buscar_perfil_gmm(
    nombre_input: str | None,
    perfiles: dict[str, RefereeGMMParams],
) -> RefereeGMMParams | None:
