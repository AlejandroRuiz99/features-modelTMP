...
def _row_to_supabase(row: dict, season: int) -> dict | None:
    ...
    return {
        ...
        "fouls_home": int(hf),
        "fouls_away": int(af),
        # fouls_total es columna generada en Supabase (fouls_home + fouls_away), no insertar
        "yellows_home": int(hy),
        "yellows_away": int(ay),
...
def download_season(season: int) -> list[dict]:
    ...
    logger.info("Descargando temporada %d/%d...", season, season + 1)
    ...
    logger.info("%d partidos en el CSV.", len(rows))
    return rows
