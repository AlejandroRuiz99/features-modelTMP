...
def _calendar_url(season_start: int) -> str:
    ...
    return (
        f"{FBREF_BASE}/en/comps/12/{slug}/schedule/{slug}-La-Liga-Scores-and-Fixtures"
    )
...
def _scrape_season(driver, season_start: int, already_done: set) -> tuple[int, int]:
    ...
    if not _wait_for_page(driver):
        logger.warning(
            "No se pudo pasar Cloudflare en calendario para temporada %s", season_label
        )
        return 0, 0
    ...
    for i, (cal_date, cal_home, cal_away, link) in enumerate(match_entries):
        ...
        logger.debug("[%d/%d] %s vs %s", i + 1, total, cal_home, cal_away)
        try:
            driver.get(link)
            if not _wait_for_page(driver):
                logger.warning("Cloudflare bloqueado: %s", link)
                continue
            ...
            teams = (
                [
                    a.text.strip()
                    for a in scorebox.select('strong > a[href*="/squads/"]')
                ]
                if scorebox
                else []
            )
            ...
            if not team_stats:
                logger.debug("Sin team_stats: %s vs %s", home_name, away_name)
                continue
            ...
            if not found:
                logger.debug("Sin posesion en pagina: %s vs %s", home_name, away_name)

        except Exception as e:
            logger.warning("Error procesando partido: %s", e, exc_info=True)
...
def scrape_possession(seasons: list[int] | None = None) -> int:
    ...
    logger.info("Consultando que temporadas necesitan posesion...")
    missing_by_season = _seasons_with_missing_possession()
    if not missing_by_season:
        logger.info("Todo completo, no hay nada que scrapear.")
        return 0
    for s, n in sorted(missing_by_season.items()):
        logger.info("   %s: %d partidos sin posesion", s, n)
    ...
    if not seasons_needed:
        logger.info("Las temporadas con datos faltantes no estan en la configuracion.")
        return 0

    logger.info("Temporadas a procesar: %s", [_season_label(s) for s in seasons_needed])

    logger.info("Consultando partidos con posesion ya en Supabase...")
    already_done = get_matches_with_possession()
    logger.info("%d partidos ya tienen posesion.", len(already_done))
    ...
    logger.info(
        "Total: %d partidos actualizados, %d ya tenian posesion.",
        total_written,
        total_skipped,
    )
