"""Interfaz publica del modulo featuresGenerator.
    ...
    *,
    ...
    Args:
        ...
        features_config:   Ruta a un YAML alternativo o dict directo. Si None, usa
                           features.yaml del directorio de featuresGenerator.
    """
    state = get_state(refresh=refresh_data)
    return build_features(**_build_kwargs(
        equipo_local, equipo_visitante, state,
        jornada=jornada, arbitro=arbitro,
...
def generate_features_batch(
    ...
    *,
    ...
    state = get_state(refresh=refresh_data)
    return [
        build_features(**_build_kwargs(
            p["equipo_local"], p["equipo_visitante"], state,
