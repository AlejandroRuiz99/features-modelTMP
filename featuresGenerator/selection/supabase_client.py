...
def _paginate_select(sb: Client, table: str, columns: str, *, query_fn=None, **kwargs) -> list[dict]:
...
def get_matches_with_possession() -> set[tuple[str, str, str]]:
    ...
    rows = _paginate_select(
        sb, "matches", "match_date,home_team,away_team",
...
def fetch_laliga_objectives() -> dict[str, dict]:
    ...
    sb = get_client()
    rows = sb.table("laliga_objectives").select(
        "equipo,categoria,num_categoria,competiciones_activas"
    ).execute()
...
def fetch_latest_odds_rows(sport: str = "soccer") -> dict:
    ...
    """
    sb = get_client()
    latest_resp = sb.table("odds_raw").select("scraped_at").order("scraped_at", desc=True).limit(1).execute()
    ...
    while True:
        q = (
            sb.table("odds_raw")
            .select("home_team,away_team,mercado,selection,cuota,external_event_id,partido,sport,scraped_at")
