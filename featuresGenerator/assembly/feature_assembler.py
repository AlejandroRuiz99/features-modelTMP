...
def _ctx_payload_to_comp(ctx_payload: dict) -> dict:
    ...
    def _side(data: dict) -> dict:
        ...
        factors = compet.get("factors", {"xg_factor": 1.0, "xfouls_factor": 1.0, "posesion_delta_pp": 0.0})
        lectura = (
            "foco competitivo alto en liga"                        if icc is not None and icc >= 0.20
            else "foco de liga condicionado por otras prioridades"  if icc is not None and icc <= -0.20
...
def _compute_expected_stats(
    ...
    kp_kwargs = dict(
        equipo_local=eq_local, equipo_visitante=eq_visit,
        partidos=partidos, xstyles=xstyles, xfouls_result=xf,
        ref_perfiles=refs, arbitro=arbitro, jornada=jornada,
    )
    ctx_kwargs = dict(
        state=state, eq_local=eq_local, eq_visit=eq_visit,
        jornada=jornada, arbitro=arbitro, arbitraje_source=arbitraje_source,
        match_date=fecha_partido,
    )

    # Pasada 1: sin ICC → contexto de temporada → calcular ICC real
    kp_initial  = ensamblar_knowledge_pack(**kp_kwargs, contexto_comp=None)
    ctx_payload, _ = build_context_payload(
        **ctx_kwargs, contexto_temporada=kp_initial.get("contexto_temporada", {}),
    )

    # Pasada 2: ICC pre-computado → ajustar metricas
    ctx_comp = _ctx_payload_to_comp(ctx_payload)
    kp_final = ensamblar_knowledge_pack(**kp_kwargs, contexto_comp=ctx_comp)
...
def _fetch_and_apply_market(
    ...
    eq_local: str,
    eq_visit: str,
    cuotas_prepartido: Optional[dict],
    skip_market_fetch: bool,
    warnings: list[str],
) -> tuple[dict, Optional[dict], list[str], Optional[str]]:
    ...
    if skip_market_fetch:
        market_category = {"catalogo_disponible": [], "traza": {"odds_scraped_at": None}}
        market_input_model = None
        market_used: list[str] = []
        odds_scraped_at = None
    else:
        market_category, market_input_model, market_used, odds_scraped_at = build_market_category(
            scores=scores,
            eq_local=eq_local,
            eq_visit=eq_visit,
    ...
    if market_input_model:
        ...
        warnings.append("No se encontraron cuotas compatibles en odds_raw para este partido.")
...
def _h2h_faltas(partidos: list[dict], eq_local: str, eq_visit: str) -> tuple[float | None, int]:
...
def _assemble_contract(
    ...
    jornada: int,
    fecha_partido: Optional[str],
    market_input_model: Optional[dict],
    market_used: list[str],
    odds_scraped_at: Optional[str],
    warnings: list[str],
) -> dict:
    ...
    comp_local = ctx_payload.get("competitivo", {}).get("local", {}).get("competitividad", {})
    comp_visit = ctx_payload.get("competitivo", {}).get("visitante", {}).get("competitividad", {})
    ...
    contract = {
        ...
        "metricas_esperadas": {
            ...
            "agresividad": {
                ...
                "total": round(max(0.0, min(1.0, (agg_l + agg_v) / _AGG_SCALE_TOTAL)), 4),
            },
            "volumen": {
...
def _flatten(raw: dict) -> dict:
    ...
    return {
        ...
        "away_fouls_committed_curr": float(visitante_temp.get("faltas_cometidas", 12.0)),
        ...
        "away_corners_curr": float(visitante_temp.get("corners", pace_index_val * 0.15)),
        ...
        "xg_diff": float(xgoles.get("local", 0.0)) - float(xgoles.get("visitante", 0.0)),
        ...
        "market_favorite_prob": float(derivadas.get("market_favorite_prob", max(p_h, p_a))),
        ...
        "intensidad_esperada": str(contexto_partido.get("intensidad_esperada", "media")),
        "riesgo_disciplinario": str(contexto_partido.get("riesgo_disciplinario", "medio")),
...
def _make_registry(
    features_profile: Optional[str],
    features_config: Optional[Union[str, dict]],
) -> FeatureRegistry:
...
def _run_pipeline(
    *,
    ...
    arbitro_input: Optional[str],
    arbitraje_source: str,
    cuotas_prepartido: Optional[dict[str, Any]],
    skip_market_fetch: bool,
    fecha_partido_input: Optional[str],
) -> dict[str, Any]:
    ...
    eq_local, eq_visit, arbitro, jornada_final, warnings = _resolve_inputs(
        state, equipo_local_input, equipo_visitante_input, arbitro_input, jornada,
    )
    kp, _, xf = _compute_expected_stats(
        state, eq_local, eq_visit, arbitro, jornada_final,
        arbitraje_source, fecha_partido_input,
    )
    kp, market_input_model, market_used, odds_scraped_at = _fetch_and_apply_market(
        state, kp, eq_local, eq_visit, cuotas_prepartido, skip_market_fetch, warnings,
    )
    return _assemble_contract(
        state, kp, eq_local, eq_visit, arbitro, arbitraje_source,
        jornada_final, fecha_partido_input,
        market_input_model, market_used, odds_scraped_at,
        warnings,
    )
...
def build_features(
    *,
    ...
    arbitro_input: Optional[str] = None,
    arbitraje_source: str = "unknown",
    cuotas_prepartido: Optional[dict[str, Any]] = None,
    skip_market_fetch: bool = False,
    fecha_partido_input: Optional[str] = None,
    features_profile: Optional[str] = None,
    features_config: Optional[Union[str, dict]] = None,
) -> dict[str, Any]:
