...
from core.utils import dedupe_dicts, extract_ou, no_vig, no_vig_3, market_entropy, norm_text, safe_float, team_match
...
def _classify_ou_market(nm: str) -> Optional[str]:
...
def _pick_best_line(
    ...
    complete = [(ln, odds) for ln, odds in lines_by_value.items()
                if "over" in odds and "under" in odds]
    partial = [(ln, odds) for ln, odds in lines_by_value.items()
               if "over" in odds]
...
def build_market_input_model(
    ...
    ou_lines: dict[str, dict[float, dict[str, float]]] = defaultdict(lambda: defaultdict(dict))
    for r in rows:
        nm = norm_text(r.get("mercado"))
        odd = safe_float(r.get("cuota"))
        if odd is None:
...
def build_market_category(
    *,
    ...
    available = sorted({
        name for r in rows
        if (name := (r.get("mercado") or "").strip())
    })
    market_input_model, used = build_market_input_model(
        rows, eq_local=eq_local, eq_visit=eq_visit,
    )
