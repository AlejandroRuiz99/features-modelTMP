...
def team_match(a: Optional[str], b: Optional[str]) -> bool:
    na = norm_text(a)
    nb = norm_text(b)
    if not na or not nb:
        return False
...
def extract_ou(selection: Optional[str]) -> tuple[Optional[str], Optional[float]]:
...
def classify_competition(competition: Optional[str]) -> str:
