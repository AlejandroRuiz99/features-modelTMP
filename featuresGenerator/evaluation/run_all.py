...
from evaluation import xfouls_eval, referee_eval, iap_eval, xgoals_eval, xposesion_eval, aggressivity_eval
...
def evaluar_todos(
    ...
    if do_calibrar:
        ...
        if update_config:
            ...
            if cal_xf["best_mae"] < mae_actual:
                xfouls_eval._update_config(cal_xf["best_decay_lambda"], cal_xf["best_alpha_card_pressure"])
                print(f"  [OK] config.yaml → λ={cal_xf['best_decay_lambda']}  α={cal_xf['best_alpha_card_pressure']}")
            else:
    ...
    if do_calibrar:
        ...
        if update_config:
            ...
            if cal_iap["best_r"] > r_actual:
                iap_eval._update_config(cal_iap["best_peso_faltas"], cal_iap["best_peso_amarillas"])
                print(f"  [OK] config.yaml → peso_faltas={cal_iap['best_peso_faltas']}  peso_amarillas={cal_iap['best_peso_amarillas']}")
            else:
    ...
    if do_calibrar:
        cal_agg = aggressivity_eval.calibrar_agresividad_volumen(partidos, n_ultimos=n_ultimos)
...
def main():
    ...
    args = eval_cli("Validación y calibración completa del featuresGenerator", calibrable=True)
...
def _print_summary(s: dict) -> None:
    header("RESUMEN — VALIDACIÓN DEL FEATURESGENERATOR")
