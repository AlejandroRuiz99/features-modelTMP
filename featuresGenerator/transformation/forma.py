...
def calcular_forma_reciente(
    ...
    if not ultimos:
        return {
            "partidos_analizados": 0,  "puntos": 0, "puntos_media": 0.0,
            "victorias": 0, "empates": 0, "derrotas": 0,
            "goles_anotados_media": 0.0, "goles_recibidos_media": 0.0,
            "faltas_media": 0.0, "tarjetas_media": 0.0,
            "racha": [], "racha_str": "—", "tendencia": "sin_datos",
        }
    ...
    for p in ultimos:
        ...
        racha.append(res)
        puntos += _PTS[res]
        if res == "W":   victorias += 1
        elif res == "D": empates   += 1
        else:            derrotas  += 1
    ...
    if n_real >= 4:
        ...
        if max_pts > 0:
            ...
            if diff > 0.20:    tendencia = "mejorando"
            elif diff < -0.20: tendencia = "empeorando"
...
def calcular_contexto_temporada(
    ...
    if jornada <= 10:   tramo, tramo_desc, presion = "inicio", "inicio de temporada (rodaje)",  "baja"
    elif jornada <= 26: tramo, tramo_desc, presion = "medio",  "tramo central de la temporada", "media"
    else:               tramo, tramo_desc, presion = "final",  "recta final de la temporada",   "alta"
