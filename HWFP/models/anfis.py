...
class ANFISFoulPredictor:
    ...
    def __init__(
        ...
        output_scale: float = 18.0,
        output_tanh_temp: float = 4.0,
        match_intensity_weights: list[float] | None = None,
        play_style_weights: list[float] | None = None,
    ):
        ...
        self._feature_mins: np.ndarray | None = None
        self._feature_maxs: np.ndarray | None = None
    ...
    def _extract_features(self, match: dict) -> np.ndarray:
        ...
        rank_diff = abs(
            match.get("home_rank_curr", 10) - match.get("away_rank_curr", 10)
        )
