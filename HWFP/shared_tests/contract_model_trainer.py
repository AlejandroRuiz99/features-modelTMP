...
def model_trainer(request):
    ...
    pytest.importorskip("HWFP.training.adapters.pytorch_model_trainer")
...
def test_model_trainer_stub_raises_not_implemented():
    mod = pytest.importorskip("HWFP.training.adapters.pytorch_model_trainer")
    adapter = mod.PyTorchModelTrainer()
    with pytest.raises(NotImplementedError):
        adapter.fit(_EXAMPLES, _HYPERPARAMS)
