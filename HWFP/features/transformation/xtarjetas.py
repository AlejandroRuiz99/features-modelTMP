...
def calcular_xtarjetas(
    ...
    return {
        ...
        "xrojas_total": round(
            safe(sl.get("rojas"), 0.04) + safe(sv.get("rojas"), 0.04), 3
        ),
...
def calcular_agresividad_volumen(xfouls_result: dict, xtarjetas: dict) -> dict:
    ...
    ya_l = safe(
        xtarjetas.get("xamarillas_local"), safe(xtarjetas.get("xtarjetas_local"), 0.0)
    )
    ya_v = safe(
        xtarjetas.get("xamarillas_visitante"),
        safe(xtarjetas.get("xtarjetas_visitante"), 0.0),
    )
